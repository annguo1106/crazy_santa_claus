...
void model_setup()
{
    ...
    helicopter.object->generateParticlesFromVertices();
    // cerr << "Particle size: " << helicopter.object->particles.size() << endl;
...
void shader_setup()
{
    ...
    std::vector<std::string> shadingMethod = {
        ...
        "bling-phong",
        "melting", // addional shading effects (advanced)
        "particleSys",
    };
...
void update()
{
    ...
    if (isMelting)
    {
        helicopter.object->updateParticles(deltaTime);
    }
...
void render()
{
    ...
    if (shaderProgramIndex == 1)
    {
        ...
        shaderPrograms[shaderProgramIndex]->set_uniform_value("cameraPos", cameraModel[3]); // camera position
        // texture
    }
    if (shaderProgramIndex == 2)
    {
        shaderPrograms[shaderProgramIndex]->set_uniform_value("time", timing);
    ...
    else if (shaderProgramIndex == 3)
    {
        ...
        for (const auto &particle : helicopter.object->particles)
        {
            // cerr << "Particle position: " << particle.position.x << " " << particle.position.y << " " << particle.position.z << endl;
            cerr << "Particle color: " << particle.color.x << " " << particle.color.y << " " << particle.color.z << " " << particle.color.w << endl;
            if (particle.life > 0.0f)
            {
                cerr << "Particle life: " << particle.life << endl;
                shaderPrograms[shaderProgramIndex]->set_uniform_value("particlePosition", particle.position);
                shaderPrograms[shaderProgramIndex]->set_uniform_value("particleColor", particle.color);

                glPointSize(5.0f); // 粒子大小
...
int main()
{
    ...
    GLFWwindow *window = glfwCreateWindow(SCR_WIDTH, SCR_HEIGHT, "HW3-111550168", NULL, NULL);
    if (window == NULL)
    {
...
void keyCallback(GLFWwindow *window, int key, int scancode, int action, int mods)
{
    ...
    {
        shaderProgramIndex = 2; // melting
    }
    if (key == GLFW_KEY_3 && (action == GLFW_REPEAT || action == GLFW_PRESS))
    {
        isMelting = !isMelting;
        shaderProgramIndex = 3; // particle
    }
